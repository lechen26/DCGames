...
public class View2048 extends Observable implements View, Runnable {
	...
	public void gameWon() {
		display.syncExec(new Runnable() {
			public void run() {
				shell.update();
				// Current board location and Preparation for Transparent Shell
				// addition
				Point location = new Point(shell.getLocation().x
						+ board.getLocation().x, shell.getLocation().y
						+ board.getLocation().y);
				String[] msgs = { "You won the game!", "now go to Study!" };
				new TransparentShell(display, msgs, board.getBounds(), location)
						.run();
				// MessageBox
				MessageBox end = new MessageBox(shell, SWT.ICON_QUESTION
						| SWT.YES | SWT.NO);
